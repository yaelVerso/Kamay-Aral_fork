...
export default function AdminModulesCsvImportDialog() {
  ...
  async function importRows(rows: CsvRow[]): Promise<Summary> {
    ...
    // Matched by title against every existing module (active or archived), so
    // re-importing an export reuses rows instead of creating a duplicate that
    ...
    const { data: existingSubmodules } = existingModuleIds.length > 0
      ? await supabase.from('admin_submodules').select('id, module_id, title, order').in('module_id', existingModuleIds)
      : { data: [] }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!moduleId) {
        const { data: newModule, error } = await supabase.from('admin_modules').insert({
          title: moduleTitle,
      ...
      if (!submoduleId) {
        ...
        const { data: newSubmodule, error } = await supabase.from('admin_submodules').insert({
          module_id: moduleId,
      ...
      const { error: signError } = await supabase.from('admin_signs').insert({
