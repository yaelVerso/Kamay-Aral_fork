...
/**
 * Manages extra video variations for a sign (e.g. a different signer or
 * regional variant) — the sign's own video_url stays the primary/default,
 * untouched by anything here. Purely additive on top of CustomSignDialog.
 */
export default function SignVideoVariationsDialog({ signId, signLabel, nextOrder, variations }: Props) {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      ...
      const { error } = await supabase.from('custom_sign_videos').insert({
        sign_id: signId,
      ...
      await recordAuditLog({ action: 'custom_sign.add_variation', description: `added a video variation for "${signLabel}"` })
      setVideoUrl('')
      setLabel('')
      router.refresh()
    } catch (err: unknown) {
  ...
  async function handleDelete(variationId: string) {
    ...
    try {
      const supabase = createClient()
      const { error } = await supabase.from('custom_sign_videos').delete().eq('id', variationId)
      if (error) throw new Error(error.message)
      await recordAuditLog({ action: 'custom_sign.remove_variation', description: `removed a video variation from "${signLabel}"` })
      router.refresh()
    } catch (err: unknown) {
      ...
      <DialogContent>
        ...
        <form onSubmit={handleAdd} className="space-y-3 border-t pt-3">
          <div className="space-y-1">
            <Label htmlFor="variation-label">Label</Label>
            <Input id="variation-label" value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g. Signer 2, Region: Cebu" />
          </div>
          <div className="space-y-1">
            <Label htmlFor="variation-video">Video link (YouTube)</Label>
            <Input id="variation-video" value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} placeholder="https://youtube.com/watch?v=..." required />
            {videoUrl.trim() && (
