...
export default function GenerateSectionReportButton({ sectionId, sectionName, modules }: Props) {
  ...
  async function handleGenerate() {
    ...
    setReport(null)
    try {
      setReport(await generateSectionReportAction(sectionId, [...selected]))
    } catch (err: unknown) {
  ...
  return (
    <>
      <Button variant="outline" size="sm" className="gap-1.5" onClick={openPicker}>
        <Sparkles className="h-4 w-4" /> Generate Section Report
      </Button>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="flex max-h-[80vh] max-w-lg flex-col">
          <DialogHeader>
            <DialogTitle>{sectionName} — Class Report</DialogTitle>
          </DialogHeader>
